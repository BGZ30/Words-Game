...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    L=SCRABBLE_LETTER_VALUES
    Score=0
    for i in range(len(word)):
        Score=Score+(L[word[i]])
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if sum(hand.values())==0:
            print 'Run out of letters. Total score: ' + str(totscr) + ' points.'
            totscr=0
    else:
        print 'Current Hand: ',
        displayHand(hand)
        ...
        word=str(raw_input())
        valwrd=isValidWord(word, hand,wordList)
        if valwrd == False and word != '.' :
